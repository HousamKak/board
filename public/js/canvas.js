...
class CanvasManager {
    ...
    setupInfiniteCanvas() {
      ...
      this.canvas.on('mouse:down', (opt) => {
        const evt = opt.e;
        if (evt.ctrlKey === true || this.currentTool === 'select') {
    ...
    initializeEventListeners() {
      ...
      document.getElementById('zoomOutBtn').addEventListener('click', () => this.zoom /= 1.2);
      document.getElementById('resetZoomBtn').addEventListener('click', () => this.zoom = 1);
    ...
    handleMouseDown(options) {
      if (this.currentTool === 'connector' && options.target) {
    ...
    handleMouseUp(options) {
      ...
      } else if (this.currentTool === 'connector' && this.isDrawing) {
    ...
    getStatusColor(status) {
      switch (status) {
        case 'todo': return '#c53030';
        case 'inprogress': return '#c6802c';
        case 'done': return '#276749';
    ...
    createRemoteStickyNote(data) {
      return new fabric.Textbox(data.content.text, {
        ...
        backgroundColor: data.style.backgroundColor || '#ffe066',
        borderRadius: 4,
        padding: 10,
        fontSize: 14,
        fontFamily: 'sans-serif',
